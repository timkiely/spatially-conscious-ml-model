...
base2 <- base_filt %>% 
  ...
  select(bbl, Sold, `SALE PRICE`,`BUILDING CLASS CATEGORY`:Annual_Sales, Year, best_vars$variable)
...
test_building <- base2 %>% filter(bbl=="1_829_16") %>% as.h2o()
test_building$pred <- as.numeric(predict(bst, newdata = test_building))
select(as_data_frame(test_building), `SALE PRICE`, pred) %>% mutate(SR = pred/`SALE PRICE`)
