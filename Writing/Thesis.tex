...
\bf 
Predicting Real Estate Sales in NYC: 

\Large

Combining Machine Learning with Spatial Analysis to Boost Predictive
Accuracy
...
\midrule
Annual\_Sales & Numeric & 12,012,780 & 1.60 & 8.24 & NA & 1.00 & 2,591.00 & 1.00 & 11,208,593\\
...
proces. In Stage 1, using the Random Forrest algorithm, we tested 3
competing feature engineering techniques (base, zip-code aggregation and
spatial-lag aggreation) for both a classification task (predicting the
occurance of a building sale) and a regression task (predicting the sale
price of a building). We analyzed the results of the first stage to
identify which geographies and building types our model assumptions
worked best. In Stage 2, using a high-performing subset of the modeling
data (selected using analysis from Stage 1), we compared four
...
Neural Network (ANN)--using the same 3 competing feature engineering
techniques for both classification and regression tasks. We analyzed the
performance of the different model/data combos and concluded with an
analysis of the variable importances of the top performing models for
both classification and regression.

In the first stage (Random Forrest using all data), we found that models
which utilized spatial features outperformed those models using zip-code
...
datasets tended to underperform when generalizing to the hold-out test
data, suggesting problems with overfitting.
...
models and tasks. For the classification task, the GBM algorithm ranked
highest in terms of AUC, followed by ANN and Random Forrest. For
regression, the ANN algorithm generally outperformed other algorithms
(as measured by Mean Absolute Error, Root Mean Squared Error and
R-squared) with the spatial features ANN model performing best.

We conclude that when predicting commerical real estate sales and sale
prices using machine learning, the addition of spatial lag features can
significantly boost predictive accuracy. We find that model overfitting
presents a challenge when using spatial features, but that this can be
overcome by implementing different algorithms, specifically ANN and GBM.
Finally, we find that our implementation of spatial-lag features works
best for certain kinds of residential buildings in specific geographic
areas, and we hypothesize that this is due to the assumptions made when
...
\hypertarget{results-of-random-forrest-sale-pice-regression-model}{%
\subsubsection{Results of Random Forrest Sale Pice Regression
Model}\label{results-of-random-forrest-sale-pice-regression-model}}
...
The Base modeling data set tends to outperform both enriched datasets,
...
\hypertarget{results-of-random-forrest-probability-of-sale-classification-model}{%
\subsubsection{Results of Random Forrest Probability of Sale
Classification
Model}\label{results-of-random-forrest-probability-of-sale-classification-model}}
...
the cell average, which is useful for spotting overperformers. We make
...
\hypertarget{model-comparisons-using-specific-geographies-and-building-types}{%
\subsection{Model Comparisons Using Specific Geographies and Building
Types}\label{model-comparisons-using-specific-geographies-and-building-types}}
...
the ROC curves and corrsponding AUC for each algorithm/feature set
combination.
...
\item
  ...
  that they are calculated using probabilitites and classes (0,1)
\item
  ...
  prediciting positive cases (occurances of sales) in the test data
\item
...
Feature importance is calculated for each algorthm as being proportional
