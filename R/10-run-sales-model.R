...
run_sales_model <- function(model_data_infile = "data/processing steps/p06_base_model_data.rds"
  ...
  if(file.exists(model_data_infile)){
    ...
    # FOR SALE MODEL : ONLY INTERESTED IN SALES --------------------------------
    model_data_list <- model_data_list %>% map(.f = ~filter(.x, !is.na(SALE_PRICE)))
    ...
    write_rds(train_test_data, "data/aux data/train_test_data_sales.rds")
    message("     ...done.")
    ...
    train_out_h2o <- foreach(i = 1:nrow(train_df_h2o_all)
                             ...
                             , .errorhandling = "pass") %do% {
                               ...
                               names(Y_val) <- Y_val_names <- "SALE_PRICE"
    ...
    message("Modeling output written to ", outfile)
