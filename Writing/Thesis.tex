...
Our goal is to compare ``spatially-concious'' machine learning
predictive models to traditional feature engineering techniques. To
accomplish this comparrisson, we create three separate modeling data
sets:
...
information from 2003-2017. For each of the three modeling dataset, we
...
Random Forrest algorithm to evaluate the data's suitability for our
feature engineering assumptions. In Stage 2, we create subsets of the
modeling datasets based on analysis conducted in Stage 1. We then
compare the performance of different algorithms across all modeling
...
\item
  ...
  base data
\item
  Train a Random Forrest model on all three datasets, for both
  classification and regression tasks
\item
  Evalutate the performance of the various Radom Forrest models on
  hold-out test data
\item
...
for less densley populated areas. In this paper, we present a solution
...
means that the time-complexity of our operation can be approximated as:
...
spatially-concious machine learning predictive models. In this paper, we
choose an arbitrary radius, and use a two-stage modeling process to test
the appropriateness of that assumption. In future work, implementing an
``adaptive bandwidth'' technique using cross-validation to determine the
optimal radius could be done.
...
\subsubsection{Random Forrest}\label{random-forrest}}

The Random Forest concept was proposed by Leo Breiman in 2001 as the
ensembling of prediction decision trees iteratively trained across
...
\hypertarget{feed-forward-artificial-neural-netwrok-ann}{%
\subsubsection{Feed-Forward Artificial Neural Netwrok
(ANN)}\label{feed-forward-artificial-neural-netwrok-ann}}
