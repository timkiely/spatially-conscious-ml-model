...
inequality'' (Zuk 2015). Income inequality has many dimensions: social,
economic, political, philosphical. But one key dimension remains
under-explored despite its potential for positive impact: geo-spatial.
The locations where people live (by choice or not) have a dramatic
impact on their quality of life and ability to progress. Yet, many who
are displaced by gentrification find themselves in self-reinforcing
cycles of displacment and poverty.
...
gentrification would be a valuable tool for preventing displacment at an
early stage, however, such a task has proven difficult historically.
...
predictive modeling (using Random Forrest) with ``spatial-lag'' features
...
while computationally expensive, can be used to greatly increase the
accuracy of spatially-conscious predictive models.
...
\caption{\label{tab:Prob Model AUC}\label{tab:ProbSaleModelAUC} Probability of Sale Models AUC}
\centering
\resizebox{\linewidth}{!}{
\begin{tabular}[t]{lrrr}
\toprule
...
\caption{\label{tab:Prob Model AUC Average Rank}\label{tab:} Distribution and Average Model Rank for Probability of Sale by AUC across Borough and Building Types}
\centering
\resizebox{\linewidth}{!}{
\begin{tabular}[t]{llllr}
\toprule
...
\hypertarget{conclusions-and-future-research}{%
...
\hypertarget{appendix-a-list-of-spatial-lag-features}{%
